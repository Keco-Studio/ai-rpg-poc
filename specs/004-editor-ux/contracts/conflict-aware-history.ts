/**
 * Conflict-Aware History Contracts — Spec 004: Editor UX v1
 *
 * Extends the HistoryStack with conflict detection preflight
 * and partial undo support.
 */
...
import type { HistoryEntryMeta } from './editor-state.js';
...
export interface ConflictAwareHistory {
  /**
...
export interface FilteredInverseBuilder {
   ...
   * e.g., a paintRect op → "map:{mapId}:layer:{layerId}:x{x}-{x+w}:y{y}-{y+h}"
   */
